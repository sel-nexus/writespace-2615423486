...
/** Register a local writer account and immediately create its session. */
export default function RegisterPage() {
  const session = getSession();
  const navigate = useNavigate();
  const [values, setValues] = useState({ displayName: '', username: '', password: '', confirm: '' });
  ...
  const submit = (event) => {
    ...
    setSession({ userId: user.id, username: user.username, displayName: user.displayName, role: user.role });
    navigate('/blogs');
  };

  return (
    <main className="min-h-[100dvh] bg-gradient-to-br from-indigo-600 via-violet-600 to-pink-500 p-4">
      <form onSubmit={submit} className="mx-auto mt-16 max-w-md rounded-xl bg-white p-8 shadow-xl">
        <h1 className="text-3xl font-bold text-indigo-700">Join WriteSpace</h1>
        ...
        <button className="mt-6 w-full rounded-lg bg-indigo-600 px-4 py-2 font-medium text-white" type="submit">Create account</button>
        <p className="mt-4 text-sm">
          Already registered? <Link className="text-indigo-600" to="/login">Login</Link>
        </p>
      </form>
    </main>
  );
